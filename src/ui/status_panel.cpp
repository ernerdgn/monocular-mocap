...
namespace mocap {
    ...
    void StatusPanel::render()
    {
        ...
        else
        {
            ...
            ImVec4 resColor;
            const char* tier = "";
            if (h >= 2160)      { resColor = ImVec4(0.2f, 0.8f, 1.0f, 1.0f); tier = " (4K)"; }
            else if (h >= 1080) { resColor = ImVec4(0.2f, 1.0f, 0.2f, 1.0f); tier = " (FHD)"; }
            else if (h >= 720)  { resColor = ImVec4(1.0f, 1.0f, 0.2f, 1.0f); tier = " (HD)"; }
            else                { resColor = ImVec4(1.0f, 0.4f, 0.4f, 1.0f); tier = " (SD)"; }
